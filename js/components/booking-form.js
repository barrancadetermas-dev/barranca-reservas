...
<div class="footer">
  ...
  async _submit() {
    ...
    try {
      ...
      // Botón WhatsApp para notificar a Alicia (solo en reservas nuevas)
      if (!this._editingId) {
        const guestFirst = document.getElementById('f-first-name')?.value?.trim() ?? '';
        const guestLast  = document.getElementById('f-last-name')?.value?.trim()  ?? '';
        ...
        const nts    = Math.round((new Date(co) - new Date(ci)) / 86400000);
        const adults = parseInt(document.getElementById('f-adults')?.value ?? '1');
        const kids   = parseInt(document.getElementById('f-children')?.value ?? '0');
        const pers   = adults + kids;
        const balAmt = balance > 0 ? formatARS(balance) : 'Pagado';

        const msg = encodeURIComponent(
          `🏠 *Nueva reserva — Barranca de Termas*\n` +
          `👤 ${guestFirst} ${guestLast}\n` +
          `🏡 ${unitNames}\n` +
          `📅 ${fmtD(ci)} → ${fmtD(co)} (${nts} noche${nts !== 1 ? 's' : ''})\n` +
          `👥 ${pers} persona${pers !== 1 ? 's' : ''}\n` +
          `💰 Al ingreso: ${balAmt}`
        );

        const alicia = '5493447448135';
        const wa     = `https://wa.me/${alicia}?text=${msg}`;

        // Toast con botón de WhatsApp
        const toast = document.createElement('div');
        toast.style.cssText = 'position:fixed;bottom:80px;left:50%;transform:translateX(-50%);' +
          'z-index:9999;background:#25D366;color:#fff;border-radius:14px;padding:12px 18px;' +
          'display:flex;align-items:center;gap:10px;box-shadow:0 8px 32px rgba(0,0,0,.25);' +
          'font-family:system-ui,sans-serif;font-size:.88rem;font-weight:600;white-space:nowrap;' +
          'cursor:pointer;animation:slideUp .25s ease';
        toast.innerHTML =
          ...
          '<path d="M11.999 2C6.477 2 2 6.477 2 12c0 1.89.524 3.656 1.438 5.168L2.05 22l4.979-1.349A9.958 9.958 0 0012 22c5.523 0 10-4.477 10-10S17.523 2 12 2zm0 18.182a8.182 8.182 0 01-4.162-1.133l-.298-.177-3.09.837.822-3.038-.194-.313A8.182 8.182 0 1112 20.182z"/>' +
          '</svg>' +
          'Notificar a Alicia por WhatsApp';

        toast.addEventListener('click', () => { window.open(wa, '_blank'); toast.remove(); });
        document.body.appendChild(toast);
        setTimeout(() => toast?.remove(), 12000); // auto-cierra en 12s
      }
