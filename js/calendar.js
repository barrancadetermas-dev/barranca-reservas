...
export class Calendar {
  ...
  _render(cellMap, reminderMap) {
    ...
    this.ctx.units.forEach(unit => {
      ...
      for (let d = 1; d <= daysInMonth; d++) {
        const dateISO  = `${this.year}-${String(this.month+1).padStart(2,'0')}-${String(d).padStart(2,'0')}`;
        const isToday  = d===todayDay && this.month===todayMonth && this.year===todayYear;
        const isPast   = !isToday && dateISO < `${todayYear}-${String(todayMonth+1).padStart(2,'0')}-${String(todayDay).padStart(2,'0')}`;
        const bookings = cellMap[unit.id]?.[d] ?? [];
        const rems     = reminderMap[dateISO] ?? [];

        const cell = document.createElement('div');
        cell.className = `cal-cell${isToday?' today-col':''}${isPast?' past-cell':''}`;
        cell.dataset.day    = d;
  ...
  _renderDualLegend() {
    ...
    // Sección 1: Estado base (sin canal)
    const statusBase = [
      { color: '#EAB308', label: 'Sin seña (pendiente)' },
      { color: '#DC2626', label: 'Con seña / depósito' },
      { color: '#16A34A', label: 'Pagado' },
      { color: '#374151', label: 'Bloqueo' },
    ];

    // Canales dinámicos desde SOURCE_CONFIG (única fuente de verdad)
    const channelItems = Object.entries(SOURCE_CONFIG).map(([key, cfg]) => ({
      color: cfg.color ?? cfg.dot ?? '#64748B',
      label: cfg.label,
    }));
    ...
    container.innerHTML = `
      <div class="cal-legend-wrapper">
        <div class="cal-legend-section">
          <div class="cal-legend-title">Estado de pago</div>
          ${statusBase.map(i => `
        ...
        <div class="cal-legend-section">
          <div class="cal-legend-title">Canal de reserva</div>
          ${channelItems.map(i => `
            <div class="legend-item">
              <div class="legend-swatch" style="background:${i.color}"></div>
              <span>${i.label}</span>
  ...
  _renderWeekView(bookings) {
    ...
    this.ctx.units.forEach(unit => {
      ...
      days.forEach(d => {
        ...
        const cell = document.createElement('div');
        const isPastWeek = !isToday && iso < today;
        cell.className = `cal-cell week-cell${isToday?' today-col':''}${isPastWeek?' past-cell':''}`;
