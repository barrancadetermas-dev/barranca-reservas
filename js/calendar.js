...
export class Calendar {
  ...
  async _openDetailById(bookingId) {
    if (!bookingId) return;
    try {
      const { data: b, error } = await this.bookingForm.db
        .from('bookings')
        .select('*, guests!bookings_guest_id_fkey(*), booking_units(unit_id, units(name)), payments(*)')
        .eq('id', bookingId).single();
      if (error) throw error;
      if (!b) { showToast('No se encontró la reserva', 'error'); return; }
      // Bloqueos: abrir panel de detalle (que tiene botón Eliminar)
      if (b.status === 'blocked' || b.is_blocked) {
        this.bookingForm.openDetail(b);
      } else {
